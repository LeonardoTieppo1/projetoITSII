...
const TaskModel = {
    ...
    updateTaskStatus: (status, id_task, callback) => {
        connection.query("UPDATE tasks SET task_status = ?, updated_at = NOW() WHERE id = ?", [status, id_task], callback);
    },
    createTask: (id_user, task_text, callback) => {
        connection.query("INSERT INTO tasks (id, id_user, task_text, task_status, created_at, updated_at) VALUES (0, ?, ?, 'new', NOW(), NOW())", [id_user, task_text], callback);
    },
    ...
    updateTaskText: (task_text, id_task, callback) => {
        connection.query("UPDATE tasks SET task_text=?, updated_at=NOW() WHERE id = ?", [task_text, id_task], callback);
    },
